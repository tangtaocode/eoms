...
public class GlobalExceptionHandler {
    ...
    public EomsResponse<?> validationBodyException(MethodArgumentNotValidException exception) {
        BindingResult result = exception.getBindingResult();
        List<ValidErrors> validErrors = new ArrayList<ValidErrors>();
        if (result.hasErrors()) {
            List<ObjectError> errors = result.getAllErrors();
            errors.forEach(p -> {
                FieldError fieldError = (FieldError)p;
                ValidErrors validError = new ValidErrors();
                validError.setField(fieldError.getField());
                validError.setMessage(fieldError.getDefaultMessage());
                validError.setObjectName(fieldError.getObjectName());
                validErrors.add(validError);
                log.error("Data check failure : object:{},field:{},errorMessage:{}", fieldError.getObjectName(),
                    fieldError.getField(), fieldError.getDefaultMessage());
            });
        }

        return EomsResponse.createdErrorEomsResponse().buildMsg(FastJsonUtils.toJSONString(validErrors));
    }
